from backbones.wan_wrapper import WanDiffusionWrapper
...
class SelfForcingTrainingPipeline:
    def __init__(self,
        ...
        if "2.1" in self.generator.model_name:
    ...
    def inference_with_trajectory(
        ...
        for block_index, current_num_frames in enumerate(all_num_frames):
            ...
            for index, current_timestep in enumerate(self.denoising_step_list):
                ...
                # additional, 这里要特别注意wan2.2有图片首帧的时候，mask2的处理，会有点不一样。这里写死了zero=False
                if "2.2" in self.generator.model_name:
                    # 这里如果不用图片，zero应设置为false
                    mask1, mask2 = masks_like(noisy_input, zero=False)
                    mask2 = torch.stack(mask2, dim=0) # torch.Size([1, 30, 48, 44, 80])

                    temp_ts = (mask2[:, :, 0, ::2, ::2] * timestep.unsqueeze(-1).unsqueeze(-1).to(device=noisy_input.device, dtype=noisy_input.dtype))
            ...
            if "2.2" in self.generator.model_name:
                # 这里如果不用图片，zero应设置为false
                mask1, mask2 = masks_like(denoised_pred, zero=False)
                mask2 = torch.stack(mask2, dim=0) # torch.Size([1, 30, 48, 44, 80])

                temp_ts = (mask2[:, :, 0, ::2, ::2] * context_timestep.unsqueeze(-1).unsqueeze(-1).to(device=noisy_input.device, dtype=noisy_input.dtype))
                temp_ts = temp_ts.reshape(temp_ts.shape[0], -1)
                # temp_ts = torch.cat([temp_ts, temp_ts.new_ones(self.generator.seq_len - temp_ts.size(1)) * wan22_input_timestep], dim=1)
                wan22_input_timestep = temp_ts.to(noise.device, dtype=torch.long)
            else:
        ...
        for current_num_frames in all_num_frames:
            ...
            for index, current_timestep in enumerate(self.denoising_step_list):
                ...
                if "2.2" in self.generator.model_name:
                    ...
                    temp_ts = (mask2[:, :, 0, ::2, ::2] * timestep.unsqueeze(-1).unsqueeze(-1).to(device=noise.device, dtype=noise.dtype))
                    temp_ts = temp_ts.reshape(temp_ts.shape[0], -1) # torch.Size([1, 15004])
                    # temp_ts = torch.cat([temp_ts, temp_ts.new_ones(temp_ts.shape[0], self.model.generator.seq_len - temp_ts.size(1)) * wan22_input_timestep.unsqueeze(-1)], dim=1)
                    wan22_input_timestep = temp_ts.to(noise.device, dtype=torch.long)
                else:
                    mask1, mask2 = None, None
                    wan22_input_timestep = None
